...
export class AlphaColorDirective implements OnInit, OnChanges {
  ...
  private onChanged(): void {
    ...
    this.stylePropertyNames.forEach((stylePropertyName) => this.element.style.removeProperty(stylePropertyName))
    ...
    this.stylePropertyNames.forEach((stylePropertyName) => {
      ...
      this.element.style[stylePropertyName] = `rgba(${color.r},${color.g},${color.b},${color.a * this.alpha})`
    })
  }
  ...
  private parseColor(input: string): RGBA {
    if (input.substr(0, 1) == "#") {
      ...
      const colors = input.split("(")[1].split(")")[0].split(",").map(x => +x)
      return {
        r: colors[0],
        g: colors[1],
        b: colors[2],
